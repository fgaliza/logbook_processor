...
def test_logbook_stream_processor_distance_smaller_than_fifteen(logbook_stream_processor):
    last_valid_waypoint = Waypoint(lat=1, timestamp="2019-05-23T00:00:00Z", lng=1)
    logbook_stream_processor.last_valid_waypoint = last_valid_waypoint
...
def test_logbook_stream_processor_return_trip(logbook_stream_processor):
    ...
    assert response.start is last_valid_waypoint
